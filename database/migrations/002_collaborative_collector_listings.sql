-- MangaMarketplace Phase 1 collaborative collector listings.
--
...
-- The creator organizes the offer; item ownership and seller proceeds are
-- represented independently by listing_contributors.
...
-- Phase 1 has no order/payment record capable of proving a purchase. Keep the
-- terminal status unavailable until Phase 2 replaces this guard with an
-- order-backed transaction that also retires every overlapping offer.
...
comment on table public.listing_contributors is
  'Per-seller consent and gross proceeds allocation for a collaborative listing.';
